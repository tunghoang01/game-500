...
async function getNameOfItem(item, file) {
  ...
  $(".shared-item_cards-item_name_component__itemNameLink").each((i, element) => {
    ...
    if (i == index) {
      ...
      fs.rename(currPath, newPath, function (err) {
        if (err) {
          ...
          $(".shared-item_cards-preview_image_component__imageLink").each((j, elm) => {
            if (j == index) {
              ...
              fs.writeFile(`./json/${newPath}.json`, json, "utf-8", function (err) {
                if (err) {
